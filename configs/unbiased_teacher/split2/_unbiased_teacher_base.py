...
test_interval = 6
save_interval = 12
classes = ['airplane', 'airport', 'storagetank', 'baseballfield','golffield' , 'windmill', 'bridge', 'ship', 'basketballcourt', 'harbor']
...
data = dict(
    ...
    train=dict(
        type='SemiDataset',
        ann_file=f'./dataset/DIOR/annotations_json_split2/DIOR_ID_labeled_split2.json',
        ann_file_u=f'./dataset/DIOR/annotations_json_split2/DIOR_ID_unlabeled_split2.json',
    ...
    val=dict(
        type='CocoDataset',
        ann_file=f'./dataset/DIOR/annotations_json_split2/DIOR_test.json',
        img_prefix=data_root,
        classes=classes,
        pipeline=test_pipeline),
    test=dict(
        type='CocoDataset',
        ann_file=f'dataset/DIOR/annotations_json_split2/DIOR_ID_MIX_OOD_split2.json',
        img_prefix=data_root,
        classes=classes,
        pipeline=test_pipeline))
...
log_level = 'INFO'
resume_from = None
load_from = f'./work_dirs/split2/baseline_ssod/latest.pth'
workflow = [('train', 1)]

# # -------------------------model------------------------------
model = dict(
    type='UnbiasedTeacher',
    ema_config='./configs/baseline/ema_config/ut_ema_split2.py',
    ema_ckpt=load_from,
    cfg=dict(
        debug=False,
        score_thr=score,
